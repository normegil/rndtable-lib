...
impl TryFrom<TomlRandomTable> for RandomTable {
    ...
    fn try_from(value: TomlRandomTable) -> Result<Self, Self::Error> {
        ...
        Ok(Self{ name: value.name, entries })
    }
}
...
impl TryFrom<TomlEntry> for Entry {
    ...
    fn try_from(value: TomlEntry) -> Result<Self, Self::Error> {
        let pure_number_regex = Regex::new(r"^[0-9]+$").expect("Hardcoded regex is not correct - It's a bug !");
        let lower_bound;
        let upper_bound;
        let keys = value.keys.trim();
        if pure_number_regex.is_match(&keys) {
            lower_bound = keys.parse::<u32>()
                .map_err(|source| ConversionError::ParseSingleInt { key: keys.to_string(), source })?;
            upper_bound = lower_bound;
        } else if keys.contains("-") {
            let splitted: Vec<&str> = keys.split('-').collect();
            if splitted.len() != 2 {
                return Err(ConversionError::InvalidKeysFormat { keys: keys.to_string() });
            }
            let first_key = splitted[0].parse::<u32>()
                .map_err(|source| ConversionError::ParseMultiIntError { key: splitted[0].to_string(), source })?;
            let second_key = splitted[1].parse::<u32>()
                .map_err(|source| ConversionError::ParseMultiIntError { key: splitted[1].to_string(), source })?;
        ...
        Ok(Self { lower_bound, upper_bound, text: value.text })
    }
}